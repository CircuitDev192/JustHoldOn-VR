...
namespace VArmory
{
    public class Hand : Slot
    {
        ...
        {
            if (VArmoryInput.TouchPadInput(null, VArmoryInput.TouchPadDirection.dontMatter, this))
        ...
        protected virtual void Update()
        {
            ...
            if (pointGrab)
                if (rayGrab)
                {
                    ...
                    {
                        if(VArmoryInput.Input(touchpadInput, this))
                        {
        ...
        //Sends input to the IV with the highest priority devided by distance to the interaction sphere provided in interactions
        ...
        public virtual Slot GetClosestValidSlot(Item item)
        {
            ...
            for (int i = 0; i < potentialSlots.Count; i++)
            {
                ...
                if (closestValidSlot)
                {
                    if (!closestValidSlot.HasItem && potentialSlot.HasItem) //If the current closest slot is empty and the potential is not continue looking
                        continue;

                    if (closestValidSlot.HasItem && !potentialSlot.HasItem) //If the current closest slot has a stored item and the potenital one does not set the potential
                    {                                                       //to the current closest regardless of distance.
        ...
        //Starts interactions with the item at the top of the stack and this hand
        ...
        //Todo: Add to a stack being held by a hand
        Slot ClosestStackableSlot(Item tempItem)
        {
            ...
            for (int i = 0; i < potentialSlots.Count; i++)
            {
                ...
                if (tempSlot.HasItem)
                {
                    if (!tempSlot.StackableStoredItem)//Ignore non stackable slots
                        continue;

                    if (tempSlot.StackIsFull)//Ignore slots with full stacks
                        continue;
                }
                ...
                if (closestSlot)
                {
                    if (closestSlot.HasItem && !tempSlot.HasItem) //If the closest slot has a stored item and the potential slot is empty continue looking
                        continue;

                    if (!closestSlot.HasItem && tempSlot.HasItem) //If the current closest slot is empty and the potential slot is not set it to the closest slot regargless of distance
                    {
                        closestDistance = distance;
        ...
        //The following functions are only used by the canvas menu options
